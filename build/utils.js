...
function generateGridMatrix(layout /*: Layout*/, _ref7) /*: Array<Array<string>>*/{
  let {
    width,
    height
  } = _ref7;
...
function addGridMatrixItem(item /*: LayoutItem*/, matrix /*: Array<Array<string>>*/) /*: void*/{
  const {
    x,
    y,
    w,
...
function calculateHeightScore(matrix /*: Array<Array<string>>*/, x /*: number*/, y /*: number*/, w /*: number*/) {
  let hScore = 1;
  // if not on left edge, check left neighbor
  const xBeforeItem = x - 1;
  if (xBeforeItem >= 0) {
    hScore += scoreNeighborBoundaries(matrix, xBeforeItem, y);
  }
  // if not on right edge, check right neighbor
  const xAfterItem = x + w;
...
function generateGapItem(matrix /*: Array<Array<string>>*/, y /*: number*/, x /*: number*/, i /*: string*/, gapFillHeight /*: number*/) /*: LayoutItem*/{
  // find width
  let w = 1;
  while (matrix[y][x + w] === null && x + w < matrix[y].length) {
    w++;
  }
  ...
  const heightScores = [calculateHeightScore(matrix, x, y, w)];
  let h = 2;
  for (let currentY = y + h - 1; currentY < matrix.length; currentY++) {
    if (h === gapFillHeight + 1) break; // stop looping when we get past gapFillHeight

    // check whether this row is empty
    let dx;
    for (dx = 0; dx < w; dx++) {
      if (matrix[currentY][x + dx] !== null) break; // found something -> didn't get all the way to w
    }
    ...
    heightScores.push(calculateHeightScore(matrix, x, currentY, w));
    h++;
  }

  // use the best scoring height that we calculated
  h = heightScores.reduce((bestHeight, currentScore, i) => {
    const bestHeightIndex = bestHeight - 1;
 ...
 * @param  {Boolean} lastRowGaps  Do you want to insert a last row gap filler, and if so how many (default is 0, so none)
 * @param  {Number} gapFillHeight how tall the gap filler should grow to be
 * @throw  {Array}                The new layout
 */
function fillInGaps(layout /*: Layout*/, columnCount /*: number*/, fillGaps /*: boolean*/, lastRowGaps /*: number*/, gapFillHeight /*: number*/) /*: Layout*/{
  if (!fillGaps) return layout;
  ...
  const matrix = generateGridMatrix(layout, {
    width: columnCount,
    height: maxY
  });
  layout.forEach(item => addGridMatrixItem(item, matrix));
  const gaps = [];
  for (let y = 0; y < matrix.length; y++) {
    for (let x = 0; x < matrix[y].length; x++) {
      // add gap fillers where the matrix is empty
      if (matrix[y][x] === null) {
        const gapItem = generateGapItem(matrix, y, x, `gap-${gaps.length}`, gapFillHeight);
        gaps.push(gapItem);
  ...
  if (lastRowGaps) {
    const gapWidth = columnCount / lastRowGaps;
    for (let i = 0; i < lastRowGaps; i++) {
      gaps.push({
        w: gapWidth,
        h: gapFillHeight,
        x: i * gapWidth,
